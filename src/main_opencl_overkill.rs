...
inline dd_real two_prod(double a, double b) {
    ...
    res.lo = ((a_hi * b_hi - res.hi) + a_hi * b_lo + a_lo * b_hi) + a_lo * b_lo;
    return res;
}
...
inline dd_real dd_add(dd_real a, dd_real b) {
    ...
    double t1 = a.lo + b.lo;
    dd_real r = two_sum(s.hi, t1 + s.lo);
    return r;
}
...
inline double dd_to_double(dd_real a) {
    return a.hi + a.lo;
}
...
__kernel void mandelbrot_dd(
    ...
    dd_real center_x = {center_x_hi, center_x_lo};
    dd_real center_y = {center_y_hi, center_y_lo};
    dd_real scale = {scale_hi, scale_lo};

    int x = get_global_id(0);
    int y = get_global_id(1);
    int index = y * width + x;

    // Convert pixel position to complex coordinates using double-double precision
    dd_real fx = (dd_real){(double)x / (double)width - 0.5, 0.0};
    dd_real fy = (dd_real){(double)y / (double)height - 0.5, 0.0};

    dd_real re = dd_add(center_x, dd_mul(fx, scale));
    dd_real im = dd_add(center_y, dd_mul(fy, scale));

    dd_real zr = (dd_real){0.0, 0.0};
    dd_real zi = (dd_real){0.0, 0.0};

    int iter = 0;
    while (iter < max_iter) {
        // z = z^2 + c
        // Compute zr^2 - zi^2 + re
        dd_real zr2 = dd_mul(zr, zr);
        dd_real zi2 = dd_mul(zi, zi);
        dd_real zr_new = dd_add(dd_sub(zr2, zi2), re);

        // Compute 2 * zr * zi + im
        dd_real zrzi = dd_mul(zr, zi);
        dd_real zi_new = dd_add(dd_add(zrzi, zrzi), im);

        zr = zr_new;
        zi = zi_new;

        // |z|^2 = zr^2 + zi^2
        dd_real mag2 = dd_add(dd_mul(zr, zr), dd_mul(zi, zi));
        if (dd_to_double(mag2) > 4.0)
            break;
